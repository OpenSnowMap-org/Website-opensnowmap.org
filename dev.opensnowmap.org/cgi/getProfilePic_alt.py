...
def createPics(tracks):
    ...
    for k in range(0, len(xs) - 1):
        ...
        z = [zs[k], zs[k+1],       h,     h]
        v.append(zip(x, y, z))
    poly3dCollection = Poly3DCollection(v,facecolors=(0.4,0.4,0.4,0.5),edgecolors='none')
    dpi=100
    width=200
    height=150
    ...
    ax.set_xlim([min(xs)-(max(xs)-min(xs))/10, max(xs)+(max(xs)-min(xs))/10])
    ax.set_ylim([min(ys)-(max(ys)-min(ys))/10, max(ys)+(max(ys)-min(ys))/10])
    ...
    arrow=dict(facecolor=(0.1,0.1,0.1), edgecolor=(0.1,0.1,0.1), headwidth = 6, frac = 0.3, width=2)
